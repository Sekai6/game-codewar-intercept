...
try {
  ...
  const hiz = process.env.HIZ === "on" ? "on" : "off";
  ...
  if (hiz === "on") await page.waitForFunction(() => document.querySelector("#webgpu-lab")?.dataset.hizBaseCenter !== undefined, null, { timeout: 10_000 });
  ...
  const validHiz = hiz === "on" ? /^MIN_DEPTH_COMPUTE_\d+_LEVEL$/.test(result.hiz ?? "") && result.hizConsumer === "NONE_BUILD_ONLY" && /^depth/.test(result.hizSource ?? "") && Number(result.hizUpdates) >= 2 && Number.isFinite(hizMinimum) && hizMinimum >= 0 && hizMinimum <= 1 && Number.isFinite(hizCenter) && hizCenter > 0 && hizCenter <= 1 : result.hiz === "OFF";
