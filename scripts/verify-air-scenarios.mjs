...
const results = {};
try {
  for (const [preset, expected] of [["joint", 6], ["intercept", 4], ["strike", 2], ["sead", 2], ["soviet-sead", 2], ["fighter", 4], ["aew", 6]]) {
  ...
  if (errors.length || results.joint.total !== 6 || jointTu16Ranges.length !== 2 || jointTu16Ranges.some(range => range < 100) || !results.joint.missions.includes("escort") || !results.joint.escorts.includes("blue-A-6E") || results.intercept.total !== 4 || !results.intercept.missions.includes("intercept") || results.strike.total !== 2 || !results.strike.missions.includes("anti-ship") || results.sead.total !== 2 || (results.sead.missions.match(/:sead/g)?.length ?? 0) !== 2 || results["soviet-sead"].total !== 2 || (results["soviet-sead"].missions.match(/:sead/g)?.length ?? 0) !== 2 || results.fighter.total !== 4 || !results.fighter.missions.includes("red-MIG-29A") || !results.fighter.missions.includes("intercept") || results.aew.total!==6 || !results.aew.missions.includes("blue-E-2C") || !results.aew.missions.includes("red-TU-126") || (results.aew.missions.match(/:aew/g)?.length??0)!==2) process.exitCode = 1;
