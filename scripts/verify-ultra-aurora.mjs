...
try {
  ...
  if (versionLabel?.trim() !== "V1.15") throw new Error(`Unexpected application version: ${versionLabel}`);
  ...
  if (!extraToggleExists || versionLabel?.trim() !== "V1.15" || baselineWithoutExtra !== "false" || active.enabled !== "true" || active.layers !== 3 || active.ultra !== "active" || active.godRays !== "0.00" || disabled.ultra !== "idle" || errors.length) process.exitCode = 1;
} finally {
  await browser.close();
}
