...
try {
  ...
  await page.goto(`${baseUrl}${baseUrl.includes("?") ? "&" : "?"}shortAirValidation=1&validationTimeScale=16`, { waitUntil: "domcontentloaded", timeout: 15_000 });
  ...
  }, null, { timeout: 30_000 }); reached = true; } catch {}
  const result = await page.locator("#scene").evaluate(canvas => ({
    ...
    phases: canvas.dataset.airWeaponPhases,
    active: canvas.dataset.airWeaponsActive,
  }));
  result.reached = reached;
  result.errors = errors;
  console.log(JSON.stringify(result, null, 2));
  if (!reached || errors.length || result.rounds >= 12 || !/KSR-5|AGM-84A/.test(result.events) ||
