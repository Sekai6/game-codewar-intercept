...
try {
  ...
  let jointConditionObserved = true;
  ...
  if (!jointConditionObserved) {
    const diagnostics = await page.locator("#scene").evaluate(c => ({
      ...
      events:c.dataset.sovietSalvoEventLog,
      hardpoints:c.dataset.airHardpointStates,
      states:c.dataset.aircraftStates,
    }));
    console.error("Joint condition timeout", JSON.stringify({ diagnostics, errors }, null, 2));
