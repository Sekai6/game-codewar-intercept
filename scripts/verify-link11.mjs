...
assert.equal(network.participantStates().length, 3);
const activities = network.recentActivities(12);
assert.ok(activities.some(event => event.kind === "poll"));
assert.ok(activities.some(event => event.kind === "transmit" && event.recipientId === "blue-ncs"));
assert.ok(activities.some(event => event.kind === "deliver" && event.trackId === "TN-101"));
