...
async function capture(enabled) {
  ...
  try {
    ...
    await page.goto(`${baseUrl}${baseUrl.includes("?") ? "&" : "?"}shortAirValidation=1&bruneton=${enabled ? "on" : "off"}`, { waitUntil: "domcontentloaded", timeout: 15_000 });
    ...
    await page.waitForFunction(() => Number(document.querySelector("#scene")?.dataset.simulationElapsed ?? 0) >= 5, null, { timeout: 20_000 });
    await page.keyboard.press("Space");
    await page.waitForTimeout(180);
    await page.locator("#scene").screenshot({ path: `verification-ultra-bruneton-${enabled ? "on" : "off"}.png` });
