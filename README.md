...
Link 11/Link 16 已进入结构化 AAR：每帧保存通信年代、开关状态、网络节点与 NCS、远端估计航迹的质量/年龄/不确定度/报告源，时间线记录去重后的轮询、发送、投递、丢包和 NCS 变化。原生战术回放用琥珀/青色误差圈区分 Link 11/16 提示；这些提示始终标为 `CUE ONLY`，不等同于目标真值或武器授权。
