...
The Soviet command system has independent `SOVIET COMMAND ERA` and `SOVIET COMMAND NETWORK ENABLED` controls; it is not a red Link 11/16 clone. `EARLY COLD WAR`, `1970s OCEAN NAVY`, the default `1980s NTU OPPOSITION`, and `LATE SOVIET` declare separate GCI, Uspekh-U, Legenda, and centralized fleet-command capabilities. GCI interception control is the first operational layer: the ground controller models era-specific scan cadence, RCS fourth-root detection range, probabilistic detection, measurement error, transmission delay, message loss, and command expiry, then sends only heading/altitude/speed and a predicted intercept point to MiG-29 formation leaders. A command contains an opaque controller track number that cannot resolve the target entity. The MiG may keep its radar on standby during the distant approach, but its own N019 must establish a valid track and maintain illumination before an R-27R launch. `npm run verify:soviet-gci` covers the era and pure-logic boundary; `npm run verify:soviet-gci-runtime` serially proves `GCI command -> organic detection -> legal launch` and state cleanup when the network is disabled.
