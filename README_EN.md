...
The scenario panel exposes separate `TACTICAL NETWORK ERA` and `LINK 16 / JTIDS NETWORK ENABLED` controls. The default `NTU BASELINE` uses local sensors/legacy links and keeps Link 16 off. `JTIDS TRANSITION` admits only era-eligible F-14A terminals and no surface ship; `LINK 16 MODERNIZED` admits eligible F-14A, A-6E, and US surface participants. Turning the checkbox off clears participants, queues, remote tracks, and ship cues. Soviet Tu-16K and MiG-29A never join Link 16 in any era. `CEC ENABLED / FUTURE` is a visible but locked roadmap entry; engagement-quality CEC fusion is not implemented.
