...
## Fleet system (phase seven)

The fleet expansion now has a dedicated domain foundation. `src/ships/` creates independent `ShipCombatantInstance` objects; each ship owns its model, kinematics, hull and subsystem health, magazines, local and network tracks, and engagement ledger. `src/fleet/` owns doctrine, formation and command roles, scenario data, and `NavalForceRuntime`. The `blue-ntu-screen` remains an optional `NAVAL FORCE` setting, with the original single-ship runtime as the default. When enabled, the existing flagship runtime is adapted as OTC while the escort enters the standard frame loop as an independent model, maneuvering entity, and air-combat target. Every ship runs its own `CombatPicture` and organic sensors; station keeping is constrained by hull acceleration, deceleration, and turn rate, with explicit straggler state and capability-based OTC/AAWC/ASuW succession.

Fleet members now also own independent electronic-warfare state. Ship definitions declare ECM strength, burn-through range, SRBOC inventory and cooldown, deployment envelope, decoy RCS, and lifetime. `src/ships/electronic-warfare-runtime.ts` gates releases on the target ship's AN/SLQ-32 and Mk 36 SRBOC health and inventory. Chaff is a moving, expanding, decaying physical entity that enters the anti-ship seeker's fourth-power signal contest; `src/fleet/electronic-warfare-visuals.ts` only renders that state. Air-launched anti-ship weapons query countermeasures by stable target-ship ID, so an escort consumes only its own rounds while the adapted flagship remains on its legacy path. `SHIP ECM` and `SRBOC` remain independent controls, and no fleet EW runtime exists while optional `NAVAL FORCE` mode is disabled. `verify:ship-electronic-warfare`, `verify:fleet-electronic-warfare`, and the single-renderer `verify:fleet-scene` cover domain behavior, target routing, and UI propagation.

Each fleet member now has an organic close-in defense loop as well. `src/air/ship-defense-bridge.ts` queries every live blue ship, so an air-launched anti-ship missile assigned to an escort enters the same `DefenseTargetRegistry` instead of disappearing from a flagship-only intake. `src/ships/ciws-runtime.ts` requires the firing ship's organic missile track and checks the physical Phalanx mount, fore/aft arc, traverse error, closing speed, TTI, subsystem health, cooldown, and owned ammunition before emitting a tracer from the real mount and settling damage through the target entity. Truth cannot authorize fire, a network cue cannot replace an organic track, and `CIWS: HOLD` applies to escorts. Both CGN-9 and CG-57 carry catalog definitions so swapping the flagship does not remove close-in defense. `verify:fleet-defense-targets`, `verify:ship-ciws`, and the single-renderer `verify:fleet-scene` cover force intake, CIWS gates, and control propagation.

The tenth fleet stage adds independent zonal damage control for optional fleet members. Fire, flooding, and subsystem casualties progressively affect each ship's propulsion, sensors, fire control, ECM, SRBOC, CIWS, and launch capability; loss of critical command systems causes the existing command runtime to reassign AAWC.
Fleet observation and AAR now expose per-ship fire, flooding, damaged/failed subsystem counts, and disabled transitions without merging damage state between ships.
`src/fleet/damage-visuals.ts` drives companion smoke anchors and hull tint from damage state only; it cannot alter hit resolution, firing, or target authority.
The ASuW coordinator now creates `ASUW-*` tasks only from the force picture, an organic ship track, and owned Harpoon inventory; missile creation remains exclusively inside the owning ship's physical Mk 141 path.

Phase four adds a fleet-owned Link 11 roll-call runtime in `src/fleet/link11-runtime.ts`, independent of the optional air-combat system, and builds a cue-only force picture in `force-picture.ts`. It publishes only organic ship observations and applies NCS polling, delivery delay, loss, quality degradation, positional uncertainty, and track aging. Anonymous `FJ-xxxxxx` reports and the fused force picture always have `weaponQuality: false`; disabling the tactical data link immediately clears both. Every escort shot must follow `AAWC assignment -> shooter organic fire-control track -> own magazine/channel/illuminator -> own launcher queue -> physical departure`; the fleet layer never creates a SAM. `verify:fleet-link11` covers network semantics, while the single-renderer `verify:fleet-scene` covers live polling, delivery, cue-only authority, and disabled-state cleanup.

Phase five adds the independent `src/fleet/air-defense-coordinator.ts`. The AAWC ranks threats by classification, estimated time to arrival, track quality, uncertainty, and age, then ranks shooters by envelope, geometry, organic weapon-quality track, magazine reserve, free launch/illumination channels, fire-control and launcher health, and formation role. Its only output is a `ForceEngagementAssignment` carrying the force track, the shooter's local track, shooter, suggested weapon, and suggested shot count. It does not deduct ammunition, occupy channels, mutate the ship engagement ledger, or create a missile. A low-quality Link 11 cue may enter threat ranking but cannot produce an assignment without an organic weapon-quality track; an empty or unavailable preferred shooter is bypassed. `verify:fleet-air-defense` covers cue-only authority, local authorization, immutable resources, and shooter transfer; `verify:fleet-scene` proves the live `organic track -> AAWC assigns CG-57` sequence in the shortened validation preset.

Phase six implements the force engagement ledger and shoot-shoot-look lifecycle in `src/fleet/engagement-runtime.ts`. A task moves through `assigned -> accepted -> weapons-away -> assessing -> resolved/leaker`. Acceptance is not a launch: only a shooter-owned physical departure may call `reportForceWeaponsAway()` and add committed weapons, combined estimated PK, and predicted intercept times. The AAWC suppresses cross-ship duplication until the first salvo's predicted intercept and a six-second observation window have passed. A reported miss or partial effect, rejection, execution timeout, or observation timeout reopens the target for the original or another organically authorized shooter; a confirmed kill remains suppressed. `verify:fleet-engagements` covers duplicate suppression, acceptance/rejection, two-shot combined PK, the look window, leaker transfer, kill closure, and expiry.

Phase seven adds `src/ships/weapon-runtime.ts` and `src/ships/launcher-adapter.ts`. The local executor validates shooter ownership, an organic weapon-quality track, track age, envelope, ammunition, fire control, launch channels, and illuminators. Only then may the adapter queue an order in that ship's Mk 10 or Mk 41 state machine. Queueing reserves only the owning ship's round and local pending record; it creates neither a missile nor a weapons-away report, and cancellation returns the same ship's resources. Only a Mk 10 rail departure or Mk 41 hatch-open hot-launch callback creates the interceptor and reports each departed round. The interceptor carries its shooter ID, and midcourse updates and terminal illumination remain tied to that shooter's organic track and directors. `verify:fleet-ship-defense` covers ownership and authorization gates. The single-renderer `verify:fleet-launch-cycle` proves physical departures from CG-57 forward and aft Mk 41 cells, independent CG-57 ammunition consumption, force-ledger transition, owned launch origins, and the absence of `SHIP SAM AUTO LAUNCH`. These runtimes are constructed only when the optional `NAVAL FORCE` switch is enabled; default single-ship behavior is unchanged.
...
The scenario panel exposes an independent `ADVANCED FLIGHT AI` switch. When disabled, aircraft remain on the lower-cost `flight-dynamics.ts` point-mass envelope; the modular advanced pipeline under `src/air/ai/flight-director.ts` and `src/air/flight/` is not evaluated in the background. The advanced model uses consistent physical units for exponential atmosphere density, dynamic pressure, speed/altitude-limited available G, finite G onset, parasitic and induced drag, coordinated turn rate, climb energy loss, specific energy, and specific excess power (`Ps`). Scripts can no longer rotate an aircraft instantly: high-G defense costs speed, low high-altitude dynamic pressure limits G, and excessive AOA invokes protection or stall recovery.

The advanced path has one runtime integration boundary. Mission logic supplies an intent, `flight-director.ts` converts it to bank/load/AOA/thrust commands, and the control-law, envelope-protection, aerodynamic, and engine modules constrain and integrate the result. Longitudinal point-mass acceleration is the explicit balance of engine thrust, parasite drag, load-squared induced drag, and gravity along the flight path; density affects dynamic pressure, available G, drag, and thrust lapse. `runtime.ts` selects advanced versus compatibility execution, applies the result, and emits events. `verify:advanced-air-aerodynamics` covers high-G energy loss, climb/descent exchange, altitude effects, and low-dynamic-pressure G limits; `verify:advanced-air-flight` covers engine lag, control limits, and stall recovery; the single-renderer `verify:advanced-air-ai-toggle` proves zero advanced updates while disabled and live state growth after enabling it.

Each of the six aircraft declares reference mass, wing area, zero-lift drag, induced-drag factor, lift-curve slope, critical AOA, control response, and engine spool response in its platform data rather than an aircraft-ID branch. Every air-launched weapon declares mass and external drag index. Advanced flight sums only weapons still physically attached to hardpoints, so a legal release immediately lowers gross-mass ratio, effective stall speed, and drag. The BVR runtime gate compares the same aircraft before and after release and rejects an ammunition-only update that leaves physical store effects unchanged.

Advanced air-to-air employment computes dynamic `Rmin / Rne / Rtr / Rmax` from shooter energy, estimated target velocity and altitude, closure, track quality, and uncertainty. Routine shots require `Rtr`; defensive opportunity shots may extend to `Rmax`. After release, an active-radar weapon is supported with a crank until autonomous acquisition and may then transition to pump. AIM-7F/R-27R support remains continuous because semi-active illumination is still required. Missile-warning tracks alone drive estimated-TTI notch or close-range drag responses. The engagement ledger continues to prevent duplicate shots while a separate support-track path preserves guidance and maneuver without granting a second weapon authorization. `verify:advanced-air-bvr` and the single-renderer `verify:advanced-air-bvr-runtime` cover the pure and integrated paths.

Advanced mode also maintains a private pilot perception picture for every aircraft. AI modules receive only anonymous `P-xxxx` contacts, estimated kinematics, classification confidence, quality, uncertainty, source, and weapon-authorization state. Physical entity bindings remain private to the `runtime.ts` adapter. Lost observations extrapolate from the last estimate while quality decays and uncertainty grows; memory contacts and Link 11/16 cues cannot authorize a shot. Disabling `ADVANCED FLIGHT AI` prevents perception updates entirely. `verify:advanced-air-perception` and the single-renderer `verify:advanced-air-perception-runtime` cover truth isolation and runtime gating.

The advanced formation layer assigns lead, shooter, supporter, cover, defensive, and rejoin roles through anonymous `F-xxxx` formation tracks; a member may only receive a target it actually observes. Threat response follows `monitor -> beam/notch -> break/drag -> recover`, selecting chaff for radar threats and flares for infrared threats. Even an urgent break remains constrained by roll response, dynamic pressure, G onset, and induced drag. Pure turn-kinematics tests plus a browser-generated ACMI regression guard against single-frame right-angle turns.

`src/air/ai/mission-planner.ts` runs as an independent 0.4 Hz layer and owns the `on-station / commit / retreat / egress / return` lifecycle. It consumes ownship state, anonymous perceived contacts, friendly protected-asset state, stores, and damage without hostile entity truth. Return fuel includes transit back to the mission origin plus a 12% reserve and engine condition. AEW aircraft move their station after an observed penetration; protected-asset loss, mission-system damage, exhausted strike stores, and bingo fuel produce explicit termination reasons. Return or egress orders raised by damage and release layers are irreversible. The planner performs zero updates when `ADVANCED FLIGHT AI` is disabled.

`pilot-model.ts` defines `rookie / regular / veteran / ace` profiles plus persistent human-factor state. Skill changes reaction, perception cadence, track memory, control precision, risk tolerance, G tolerance, stress resistance, and workload capacity; it never changes thrust, aerodynamics, or sensor truth. Missile warnings first pass a stress-dependent reaction timer. Multiple contacts, weapon support, damage, and high G increase saturation, stress, and fatigue, reducing perception rate, memory, control precision, and sustainable G. Control error is deterministic from pilot ID and simulation time so seeded replay remains reproducible. The human-factor layer also performs zero updates while advanced AI is disabled.

`damage-management.ts` converts critical-system damage into flyable constraints: asymmetric engines require bounded yaw trim, flight-control and structural damage reduce bank/G limits, engine loss caps climb demand, and structural or engine-bay damage can leak fuel into the return calculation. Radar loss aborts radar-dependent missions unless an actual infrared store remains. Uncontrollable outcomes stay with the standard loss-of-control, sea-impact, and AAR chain; the AI never deletes the entity. `verify:advanced-air-damage-management` covers these rules.

Close combat is planned by `bfm-planner.ts` from anonymous estimated range, closure, angle off nose, target aspect, and relative energy. It selects one-circle, two-circle, lead pursuit, lag pursuit, scissors, or a defensive turn. Low-altitude plans command positive vertical recovery, and every bank/load request still passes pilot G tolerance, control response, dynamic pressure, AOA, and stall protection. Advanced close-range employment prefers infrared weapons and requires a guidance-specific continuous stable-shot window; a transient nose crossing or broken geometry resets it. `bfmValidation=1` is a controlled browser-verification geometry only and does not change normal scenario spawns.

The scenario panel exposes separate `TACTICAL NETWORK ERA` and `TACTICAL DATA LINK ENABLED` controls. The default `NTU BASELINE` runs US shipboard Link 11/TADIL-A and no Link 16. `JTIDS TRANSITION` retains ship Link 11 while admitting only era-eligible F-14A JTIDS terminals to Link 16. `LINK 16 MODERNIZED` admits eligible F-14A, A-6E, and US surface participants to Link 16. The master switch disconnects every data link available to that era and clears participants, queues, remote tracks, and cues. Soviet Tu-16K and MiG-29A join neither US network. `CEC ENABLED / FUTURE` is visible but locked; engagement-quality CEC fusion is not implemented.

Link 11 is a separate roll-call runtime. A live capable participant becomes Net Control Station, and the NCS polls only one unit per response window, so adding participants lengthens the full update cycle. Reports pay queue/modem delay, range and terminal-health loss, substantial uncertainty growth, quality degradation, and coalition isolation. In the `AEW` preset the E-2C joins through its AN/ARC-158 terminal and publishes only organic AN/APS-125 detections. F-14A is deliberately not fabricated as a Link 11 terminal; it receives finite-capacity, delayed, expiring Link 4A intercept commands. Network cues can bias search but can never create fire control, launch a weapon, or update a missile.

E-2C and Tu-126 live behind the dedicated `src/air/aew/` module boundary, which owns their catalog, proportioned procedural models, orbit mission, and command network. Both use the standard airborne sensor loop with 360-degree mechanical coverage, radar horizon, RCS, ECM, probabilistic detection, and track aging, and neither has weapons or hardpoints. E-2C contributes to the fleet Link 11 picture and can control at most two F-14s over Link 4A. Tu-126 has no fabricated digital tactical-track exchange: it converts organic Liana tracks into slower, less precise voice-GCI commands for one MiG-29 at a time. Both command forms carry opaque controller track numbers and intercept parameters only; fighters still need an organic weapon-quality track.

Press `N` for the independent tactical-network observer and `Shift+N` to cycle `LINK 11 / LINK 16 / SOVIET / ALL / OFF`. Amber nodes, the NCS ring, and momentary poll lines represent Link 11; cyan nodes and moving report pulses represent Link 16; in fleet `ALL` view, OTC-to-member formation lines, station-error rings, and `shooter -> track` AAWC task lines are also shown; purple vectors represent GCI commands, orange uncertainty ellipses represent Uspekh-U/Legenda target-area reports, and red markers represent fleet mission orders and salvo assignments. The observer does not draw a permanent full-mesh “network spiderweb”: it renders only events emitted by the simulation. Estimated tracks and report areas use age/quality/uncertainty-scaled bounds rather than truth boxes. Fleet task lines are observational only and never grant weapon authority. Its separate panel permanently marks the picture `CUE ONLY - NO WEAPON AUTHORITY`.
...
The Soviet command system has independent `SOVIET COMMAND ERA` and `SOVIET COMMAND NETWORK ENABLED` controls; it is not a red Link 11/16 clone. `EARLY COLD WAR`, `1970s OCEAN NAVY`, the default `1980s NTU OPPOSITION`, and `LATE SOVIET` declare separate GCI, Uspekh-U, Legenda, and centralized fleet-command capabilities. GCI models era-specific scan cadence, RCS fourth-root range, probabilistic detection, measurement error, transmission delay, loss, command life, and finite controller workload. Early voice control quantizes bearing and altitude, commands a more conservative speed, opens the fighter radar earlier, and controls fewer formations; automated eras issue finer commands to more formations. A command carries only an opaque controller track number plus altitude, speed, radar-activation range, and an uncertain intercept point. MiG-29s wait for the first command under radar silence, then perform a faster organic N019 sector search near the command basket. They follow commanded energy until satisfied, but still require an organic weapon-quality track; R-27R still requires illumination. During defensive maneuvering the AI permits an opportunity shot only with adequate reaction time and organic authorization, preferring fire-and-forget R-73 when a close threat makes continued illumination unsound. `verify:soviet-gci` covers voice quantization, workload, era differences, and truth isolation; the single-renderer runtime gate proves `command wait -> directed search -> organic detection -> physical release` and disabled-network cleanup.

Long-range maritime target indication is a separate mission-specific path. The 1970s era uses Uspekh-U; NTU and late-Soviet eras prefer Legenda during brief satellite-pass windows and fall back to Uspekh-U between passes. Each source has its own update cadence, transmission delay, reliability, report life, and uncertainty ellipse. A report contains only an opaque track number, estimated position/velocity, report time, and recommended launch region: it never enters `AirTrack` and cannot resolve the ship entity. A Tu-16K leader initially observes a bounded EMCON period and flies toward the reported launch region; loss or expiry of the report falls back to organic Rubin-1K search. KSR-5 release still requires organic ship classification, a weapon-quality track, a fire-control channel, and physical separation from `centerline-ksr`. `verify:soviet-maritime` and the single-renderer `verify:soviet-maritime-runtime` enforce these boundaries.

The centralized fleet-command net sits above target-area reporting without creating a shared fire-control picture. A live red surface platform may act as SURFACE FLAG RELAY; an air-only scenario uses an abstract FLEET COMMAND POST. Node health affects reliability, node loss stops new orders, and already delivered orders persist until their own expiry. Each Tu-16K formation receives main-effort priority, an offset approach point, an attack window, and an egress direction while referencing only the opaque Uspekh/Legenda report number. Even an early organic Rubin-1K ship track cannot release KSR-5 before the command window; prolonged command loss falls back to autonomous doctrine after a bounded delay. verify:soviet-fleet-command covers node death and command inertia, while verify:soviet-fleet-command-runtime proves “target area/organic detection -> centralized order -> legal release inside the window”.

Anti-ship salvo coordination sits between the fleet mission order and each aircraft's weapon authorization. From the anonymous target-area estimate, participant positions, and game-scaled KSR-5 speed, the coordinator assigns a wave, an aircraft-specific release time, and a common planned arrival to every live member with a ready hardpoint. An assignment carries only the source order, opaque report number, and uncertain search center—never a target entity ID. It may release a wingman's EMCON restriction and point Rubin-1K toward the reported area, but the aircraft must still make its own probabilistic detection, establish a weapon-quality track, and physically release from `centerline-ksr`. Death, empty inventory, or hardpoint loss removes the assignment. `verify:soviet-salvo` covers truth isolation, command-window timing, common arrival, and invalidation; the single-renderer `verify:soviet-salvo-runtime` proves two assignments, two organic authorizations, two physical releases, and zero assignments with Soviet command disabled.
...
│  ├─ soviet-c2/
│  │  ├─ era.ts                  # Soviet command-era capability matrix
