...
The advanced path has one runtime integration boundary. Mission logic supplies an intent, `flight-director.ts` converts it to bank/load/AOA/thrust commands, and the control-law, envelope-protection, aerodynamic, and engine modules constrain and integrate the result. `runtime.ts` selects advanced versus compatibility execution, applies the result, and emits events. `verify:advanced-air-flight` covers energy, engine lag, control limits, and stall recovery; the single-renderer `verify:advanced-air-ai-toggle` proves zero advanced updates while disabled and live state growth after enabling it.
