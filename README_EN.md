...
The scenario panel exposes an independent `ADVANCED FLIGHT AI` switch. When disabled, aircraft remain on the lower-cost `flight-dynamics.ts` point-mass envelope; the modular advanced pipeline under `src/air/ai/flight-director.ts` and `src/air/flight/` is not evaluated in the background. The first stage adds engine spool-up/down, dynamic pressure, angle of attack, sideslip, load factor, induced drag, specific energy, specific excess power (`Ps`), AOA limiting, and stall-recovery state. These per-aircraft diagnostics are intended inputs for the later dynamic-launch-zone and BVR/BFM planners.
