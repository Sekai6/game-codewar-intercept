...
## Fleet system (phase three)

The fleet expansion now has a dedicated domain foundation. `src/ships/` creates independent `ShipCombatantInstance` objects; each ship owns its model, kinematics, hull and subsystem health, magazines, local and network tracks, and engagement ledger. `src/fleet/` owns doctrine, formation and command roles, scenario data, and `NavalForceRuntime`. Phase three connects the `blue-ntu-screen` through `scene-integration.ts` as an optional `NAVAL FORCE` scenario setting. The original single-ship runtime remains the default. When enabled, the existing flagship runtime is adapted as OTC while the escort enters the standard frame loop as an independent model, maneuvering entity, and air-combat target. Every ship runs its own `CombatPicture` and maintains organic tracks; station keeping remains constrained by each hull's acceleration, deceleration, and turn rate, with explicit straggler state and capability-based OTC/AAWC/ASuW succession. NTU Link 11 remains cue-only. Escort weapons remain inhibited until per-ship magazines, launchers, illuminators, and fire-control authorization are fully instantiated. Run `verify:fleet-domain`, `verify:fleet-runtime`, and the single-renderer `verify:fleet-scene` for resource isolation, motion/sensor/succession logic, and actual scene integration.
