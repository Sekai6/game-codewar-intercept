...
export function normalizeScenarioDocument(input: unknown): ScenarioDocument {
  ...
  return deepFreeze({
    ...
    forces: ordered(clone.forces), threatWaves: ordered(clone.threatWaves ?? []),
    strikeWaves: ordered(clone.strikeWaves ?? []), commandMessages: ordered(clone.commandMessages ?? []),
    routes: ordered(clone.routes), zones: ordered(clone.zones),
