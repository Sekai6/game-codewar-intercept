...
export const AIR_WEAPONS: Readonly<Record<AirWeaponId, AirWeaponDefinition>> = {
  "AGM-45A": { id:"AGM-45A", name:"AGM-45A Shrike", targets:["ship"], guidance:"anti-radiation", minRange:18, maxRange:260, speed:8.5, boostSeconds:2.5, maxTurnRateDeg:10, seekerRange:120, seekerFovDeg:70, datalinkInterval:1, damage:18, proximityRadius:5, countermeasureResistance:.5, massKg:177, dragIndex:.02, shipDefenseTemplate:"RGM-84 Harpoon", airToAirFlight:{sustainSeconds:7,coastDragPerSecond:.032,minimumSpeedFactor:.48,maximumFlightSeconds:75,loftAltitude:38,loftTransitionRange:115} },
  "AGM-88A": { id:"AGM-88A", name:"AGM-88A HARM", targets:["ship"], guidance:"anti-radiation", minRange:20, maxRange:520, speed:9.2, boostSeconds:2.8, maxTurnRateDeg:13, seekerRange:180, seekerFovDeg:90, datalinkInterval:.6, damage:28, proximityRadius:6, countermeasureResistance:.68, massKg:360, dragIndex:.024, shipDefenseTemplate:"RGM-84 Harpoon", airToAirFlight:{sustainSeconds:14,coastDragPerSecond:.018,minimumSpeedFactor:.55,maximumFlightSeconds:125,loftAltitude:95,loftTransitionRange:260} },
  "Kh-31P-C": { id:"Kh-31P-C", name:"Kh-31P-C (late Soviet concept)", targets:["ship"], guidance:"anti-radiation", minRange:24, maxRange:420, speed:10.8, boostSeconds:3.2, maxTurnRateDeg:12, seekerRange:155, seekerFovDeg:78, datalinkInterval:.8, damage:26, proximityRadius:6, countermeasureResistance:.62, massKg:600, dragIndex:.03, shipDefenseTemplate:"Kh-22", airToAirFlight:{sustainSeconds:11,coastDragPerSecond:.023,minimumSpeedFactor:.5,maximumFlightSeconds:100,loftAltitude:62,loftTransitionRange:210}, antiShipFlight:{boostAltitude:18,cruiseAltitude:6,terminalAltitude:.8,boostSpeedFactor:.8,cruiseSpeedFactor:.98,terminalTurnFactor:1.4} },
  ...
  "R-73": { id:"R-73", name:"R-73 Archer", targets:["aircraft"], guidance:"infrared", minRange:1.5, maxRange:135, speed:11.5, boostSeconds:3.2, maxTurnRateDeg:48, seekerRange:58, seekerFovDeg:62, datalinkInterval:0.2, damage:56, proximityRadius:3.5, countermeasureResistance:0.56, massKg:105, dragIndex:.009, shipDefenseTemplate:"Kh-22", airToAirFlight:{sustainSeconds:4.5,coastDragPerSecond:.03,minimumSpeedFactor:.44,maximumFlightSeconds:30,loftAltitude:0,loftTransitionRange:0} },
  "KSR-5": { id:"KSR-5", name:"KSR-5 Kingfish", targets:["ship"], guidance:"anti-ship-radar", minRange:180, maxRange:3000, speed:10.4, boostSeconds:8, maxTurnRateDeg:7, seekerRange:125, seekerFovDeg:50, datalinkInterval:1.2, damage:48, proximityRadius:8, countermeasureResistance:0.66, massKg:3900, dragIndex:.095, shipDefenseTemplate:"Kh-22", antiShipFlight:{boostAltitude:92,cruiseAltitude:6,terminalAltitude:.8,boostSpeedFactor:.72,cruiseSpeedFactor:.94,terminalTurnFactor:1.8} },
...
const attackThrust = {...bomberThrust,militarySpeedFactor:1.42,militaryFuelMultiplier:1.7,militaryInfraredMultiplier:1.32,afterburnerSpeedFactor:1.42};
const f14Aerodynamics = {referenceMassKg:22000,wingAreaM2:52.5,zeroLiftDragCoefficient:.024,inducedDragFactor:.055,liftCurveSlopePerDeg:.098,criticalAngleOfAttackDeg:20,controlResponseSeconds:.5,engineSpoolUpSeconds:4.2,engineSpoolDownSeconds:3.1};
...
const intruderHardpoints = A6E_STRIKE_STATIONS.map((station) => ({
  id: station.id,
  position: station.position,
  compatibleWeapons: ["AGM-84A", "AGM-45A", "AGM-88A"] as const,
  releaseDelay: 0.35,
  ignitionDelay: 0.3,
}));
const fulcrumHardpoints = MIG29A_MODEL_STATIONS.map((station) => {
  ...
  return {
    id:station.id,
    position:station.position,
    compatibleWeapons:["R-27R", "Kh-31P-C"] as const,
    releaseDelay:.22,
    ignitionDelay:.15,
  };
});
export const AIR_PLATFORM_DEFINITIONS: readonly AirPlatformDefinition[] = [
  ...
  { id:"TU-16K", name:"Tu-16K Badger-G", nation:"Soviet Union", role:"Maritime strike", mission:"anti-ship", radarCrossSection:28, infraredSignature:1.35, flight:{cruiseSpeed:4.2,maxSpeed:5.4,stallSpeed:1.8,acceleration:0.45,drag:0.022,maxLoadFactor:2.5,maxRollRateDeg:38,maxPitchRateDeg:10,fuelSeconds:1300,aerodynamics:tu16Aerodynamics,thrust:bomberThrust}, sensor:{name:"Rubin-1K",range:2600,updateInterval:1.5,fieldOfViewDeg:100,precision:0.7}, ecm:{strength:0.68,burnThroughRange:48}, countermeasures:{chaff:42,flares:18,program:{chaffBurst:4,flareBurst:2,interval:0.14,cooldown:4.5,triggerTti:26}}, loadout:{...emptyLoadout(),"KSR-5":1}, fireControlChannels:{datalink:1,illumination:0}, hardpoints:badgerHardpoints, buildModel:createTu16Model, shipDefenseTemplate:"Kh-22" },
  { id:"A-6E", name:"A-6E Intruder", nation:"United States", role:"All-weather strike", mission:"anti-ship", radarCrossSection:7, infraredSignature:1, flight:{cruiseSpeed:3.8,maxSpeed:5.5,stallSpeed:1.7,acceleration:0.62,drag:0.021,maxLoadFactor:6.5,maxRollRateDeg:95,maxPitchRateDeg:23,fuelSeconds:950,aerodynamics:a6Aerodynamics,thrust:attackThrust}, sensor:{name:"AN/APQ-148",range:1200,updateInterval:1.2,fieldOfViewDeg:100,precision:0.78}, datalink:{link16:true,minimumEra:"link16-modernized",terminalName:"JTIDS Integration",terminalReliability:0.92,timeSyncQuality:0.95}, ecm:{strength:0.52,burnThroughRange:28}, countermeasures:{chaff:24,flares:24,program:{chaffBurst:2,flareBurst:2,interval:0.2,cooldown:5,triggerTti:18}}, loadout:{...emptyLoadout(),"AGM-84A":2}, fireControlChannels:{datalink:2,illumination:0}, hardpoints:intruderHardpoints, buildModel:createA6Model, shipDefenseTemplate:"Kh-22" },
  ...
  { id:"MIG-29A-SEAD", name:"MiG-29A SEAD (late Soviet concept)", nation:"Soviet Union", role:"Anti-radiation escort", mission:"sead", radarCrossSection:5.4, infraredSignature:1.25, flight:{cruiseSpeed:5.3,maxSpeed:11.2,stallSpeed:2,acceleration:1.18,drag:0.018,maxLoadFactor:9,maxRollRateDeg:150,maxPitchRateDeg:34,fuelSeconds:700,aerodynamics:mig29Aerodynamics,thrust:mig29Thrust}, sensor:{name:"N019 Rubin",range:285,updateInterval:0.9,fieldOfViewDeg:110,precision:0.8}, passiveSensors: migPassive, ecm:{strength:0.46,burnThroughRange:30}, countermeasures:{chaff:30,flares:30,program:{chaffBurst:2,flareBurst:3,interval:0.16,cooldown:4.5,triggerTti:18}}, loadout:{...emptyLoadout(),"Kh-31P-C":2,"R-73":2}, fireControlChannels:{datalink:0,illumination:1}, hardpoints:fulcrumHardpoints, buildModel:createMig29Model, shipDefenseTemplate:"Kh-22" },
  ...AEW_PLATFORM_DEFINITIONS,
];
const AIR_PLATFORM_TACTICAL_ROLES = {
  "F-14A": "fighter", "TU-16K": "bomber", "A-6E": "attack",
  "MIG-29A": "fighter", "MIG-29A-SEAD": "fighter", "E-2C": "aew", "TU-126": "aew",
...
export const AIR_PLATFORM_BY_ID = Object.fromEntries(AIR_PLATFORM_DEFINITIONS.map(d=>[
  d.id, { ...d, passiveSensors: d.id === "F-14A" ? f14Passive : d.id === "A-6E" ? a6Passive : (d.id === "MIG-29A" || d.id === "MIG-29A-SEAD") ? migPassive : undefined, tacticalRole: AIR_PLATFORM_TACTICAL_ROLES[d.id] },
