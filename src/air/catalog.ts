...
const AIR_PLATFORM_TACTICAL_ROLES: Partial<Record<AirPlatformId, NonNullable<AirPlatformDefinition["tacticalRole"]>>> = {
...
export const AIR_PLATFORM_BY_ID = Object.fromEntries(AIR_PLATFORM_DEFINITIONS.map(d=>{
  const tacticalRole = AIR_PLATFORM_TACTICAL_ROLES[d.id];
  if (!tacticalRole) throw new Error(`Missing tactical role for air platform ${d.id}`);
  return [d.id, { ...d, tacticalRole }];
