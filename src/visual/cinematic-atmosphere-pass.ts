...
const neutralClusterByPass = new WeakMap<ShaderPass, THREE.Texture>();
...
export function createCinematicAtmospherePass() {
  ...
  const neutralCluster = new THREE.DataTexture(new Uint8Array([0, 0, 0, 0]), 1, 1, THREE.RGBAFormat);
  neutralCluster.needsUpdate = true;
  const pass = new ShaderPass({
    uniforms: {
      ...
      froxelMix: { value: 0 },
      clusteredAtlas: { value: neutralCluster },
      clusteredMix: { value: 0 },
    },
    vertexShader: `varying vec2 vUv;void main(){vUv=uv;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}`,
    fragmentShader: `
      precision highp float;varying vec2 vUv;uniform sampler2D tDiffuse;uniform sampler2D cinematicLut;uniform sampler2D ultraScatter;uniform sampler2D froxelAtlas;uniform sampler2D clusteredAtlas;uniform sampler2D tDepth;uniform vec2 sunPosition;uniform float godRayStrength;uniform float chromaticAberration;uniform vec2 resolution;uniform float lutSize;uniform float ultraScatterMix;uniform float ultraTime;uniform float cameraNear;uniform float cameraFar;uniform float froxelMix;uniform float clusteredMix;
      ...
      vec4 sampleCluster(vec2 uv,float distance){float layer=clamp(sqrt(distance/900.)*23.,0.,23.);float z0=floor(layer),z1=min(23.,z0+1.);vec2 tile0=vec2(mod(z0,8.),floor(z0/8.)),tile1=vec2(mod(z1,8.),floor(z1/8.));vec2 local=clamp(uv,0.,1.);vec2 atlas0=(tile0+local)/vec2(8.,3.);vec2 atlas1=(tile1+local)/vec2(8.,3.);return mix(texture2D(clusteredAtlas,atlas0),texture2D(clusteredAtlas,atlas1),fract(layer));}
  ...
  neutralClusterByPass.set(pass, neutralCluster);
  return pass;
}

export function setCinematicClusteredLighting(pass: ShaderPass, texture: THREE.Texture | null) {
  pass.uniforms.clusteredAtlas.value = texture ?? neutralClusterByPass.get(pass);
  pass.uniforms.clusteredMix.value = texture ? 1 : 0;
}
