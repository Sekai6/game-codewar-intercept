...
The first native Hi-Z resource gate is now available with `hiz=on`. It samples the renderer-owned `DepthTexture` directly from the WebGPU backend, resolves 4x MSAA depth into mip 0, then dispatches 2x2 minimum reductions into an `r32float` storage texture down to 1x1. On the 1280x720 Edge gate this produces 11 levels; mip-0 center depth is about `0.997895` and the final minimum is about `0.997497`. The verifier requires finite non-zero source content and records the source as `depth24plus_1280X720_S4`. A prior single-sample binding silently returned zero despite no Dawn validation error, which is why content readback remains part of the gate. The resource is still labeled `hizConsumer=NONE_BUILD_ONLY`: no quality claim is made until hierarchical SSR actually consumes the mip chain.
