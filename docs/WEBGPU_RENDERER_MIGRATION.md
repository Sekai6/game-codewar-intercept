...
An initial native froxel prototype used an `80x45x32` renderer-owned `Storage3DTexture`, WebGPU compute injection for 24 lights and a 24-step TSL integration pass. Dawn validation passed and the volume contained non-zero half-float data, but Edge A/B rejected the result. Without scene-depth clipping it produced a gray veil and visible vertical cells; clipping removed those artifacts but reduced the mean sampled RGB difference to about `2.24/765`, with no material visual improvement. More importantly, density and lights were generated from normalized texture coordinates rather than reconstructed world-space positions, so the apparent fog moved with the screen. The prototype was removed. A valid replacement must reconstruct each froxel from inverse view-projection data, use logarithmic view-space depth slices, inject world-space atmosphere and scene lights, stop integration at opaque depth, and pass the same serial A/B gate without grid artifacts.
