...
- A full-scene temporal reconstruction pass renders Ultra at 0.85x internal resolution and resolves to a full-resolution half-float history target. An eight-sample Halton jitter, per-object previous-MVP velocity prepass, linear-distance disocclusion rejection, silhouette rejection, reactive masks, variance-aware 3x3 neighborhood clipping, Catmull-Rom current-frame reconstruction and bounded RCAS-style sharpening stabilize ships, aircraft, missiles, antennas, ocean and clouds without unrestricted ghost trails. A production screenshot gate rejected 0.70x because fine antennas and foam lost unacceptable detail; that scale remains blocked until the native WebGPU reconstruction kernel is stronger.
...
- Forward+ uses a 32 x 18 x 24 view-space cluster grid and accepts the strongest 64 existing scene point lights. WebGPU compute assigns explosion, missile-exhaust, flare and navigation-light radiance to clusters; the hybrid surface resolver selects the depth slice from the real scene depth texture and adds bounded local bounce before color grading. The same light samples feed froxel scattering, but the two consumers remain distinct. A cluster failure is isolated and reported without taking the rest of Ultra offline. This is a hybrid depth-aware resolver, not a claim that Three.js PBR materials have already migrated to a native WebGPU renderer.
...
- `data-web-gpu-ultra-clustered`: `OFF` or `FORWARD_PLUS_32X18X24_64`
- `data-web-gpu-ultra-clustered-lights`, `data-web-gpu-ultra-clustered-occupied`, `data-web-gpu-ultra-clustered-updates` and `data-web-gpu-ultra-clustered-error` expose allocation health and isolated failure state.
...
Run `npm run verify:webgpu-clustered` to require a real explosion light, non-empty cluster allocation and an error-free WebGPU path. Run `npm run capture:webgpu-clustered` for the fixed explosion on/off comparison. Both scripts keep one renderer process at a time.

Future renderer work is direct storage-buffer particle drawing and material-native Forward+ after a full `WebGPURenderer` switch. A parallel WebGPU overlay was rejected because WebGL depth attachments cannot be shared with its canvas; it would render particles through hulls, ocean and clouds. The renderer switch remains separate because the current GLSL post-processing stack is not directly compatible.
