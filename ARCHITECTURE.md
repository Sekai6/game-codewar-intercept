...
- `src/datalink/types.ts`: side-neutral tactical-network participant, track-report, delivery, provenance, and diagnostic contracts.
- `src/datalink/era.ts`: communication-era capability matrix and eligibility gates. `ntu-baseline` disables Link 16; `jtids-transition` permits eligible airborne terminals only; `link16-modernized` permits eligible US air and ship terminals; `cec-enabled` remains locked until a real measurement-fusion runtime exists.
- `src/datalink/link16-network.ts`: deterministic Link 16/JTIDS game-scale runtime. It owns TDMA capacity, priority slots, queue pressure, propagation delay, terminal/time-sync health, range loss, coalition isolation, copied reports, observation deduplication, and inbox delivery. It never owns target entities or weapon authorization.
- Link 16 equipment and its minimum communication era are opt-in capability data on `AirPlatformDefinition`; neither is inferred from side or nationality. F-14A becomes eligible in `jtids-transition`; the game-scaled A-6E integration and surface participant require `link16-modernized`. Tu-16K and MiG-29A do not register, transmit, receive, or accumulate Link 16 tracks. A future Soviet data-link implementation must be a separate service with its own period capabilities.
...
Link 16 preserves this boundary. Aircraft retain separate local-radar and network-track stores. Reports pay a quality, uncertainty, and age penalty and are marked `engagementQuality: "cue"`; OODA may use them to turn the sensor field of view toward a contact, but `trackSupportsWeaponAuthorization()` rejects them. Shipboard Link 16 cues may focus `CombatPicture` search bearing, but cannot create a `CombatPicture` track, reserve a launcher, or spawn an interceptor. The ship must establish its own weapon-quality radar track and proceed through the standard engagement and physical-launcher pipeline.

Era and operator controls are runtime gates, not cosmetic UI state. A configuration change resets TDMA state, pending reports, remote aircraft tracks, and ship cues before rebuilding only eligible participants. The NTU baseline therefore cannot retain a modernized track from an earlier selection. The Link 16 checkbox can disconnect an otherwise capable era without changing platform catalog data. Soviet entities remain excluded because they declare no Link 16 terminal, not because the network contains a red-side special case.

CEC is deliberately not implemented by increasing Link 16 quality. A future CEC service must exchange time-synchronized sensor measurements with covariance and calibration provenance, build a composite engagement-quality track, and declare explicit weapon/platform support. It must still preserve physical launcher, midcourse-update, illumination/seeker, and terminal-acquisition requirements.
